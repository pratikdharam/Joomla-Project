              ...
              <span class="koowa_header__item">
                <span class="koowa_wrapped_content">
                  <span class="whitespace_preserver">
                    ...
                    ?>
                    <? if (!$category->enabled) : ?>
                      <span class="label label-draft badge bg-info"><?= translate('Draft'); ?></span>
                    <? endif; ?>

                    <? // Label owner 
                    ?>
                    <? if ($params->get('show_category_owner_label', 1) && !$category->isNew() && object('user')->getId() == $category->created_by) : ?>
                      <span class="label label-success badge bg-success"><?= translate('Owner'); ?></span>
                    <? endif; ?>
                  </span>
                </span>
              </span>
    ...
    <? if (count($subcategories) || count($documents) || $params->show_document_search) : ?>
      ...
      <form action="" method="get" class="k-js-grid-controller">
        ...
        <div class="koowa_media--gallery">
          ...
          <div class="koowa_media_wrapper koowa_media_wrapper--documents view-masonry">
            <div class="koowa_media_contents items-row cols-3 row-cols-1 row-cols-sm-2 row-cols-xl-3 row">
              <?php // this comment below must stay 
              ?><!--
                            <? $count = 0; ?>
