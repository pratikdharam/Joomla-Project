...
(function ($) {
	$(document).ready(function () {
		var url = $('div#userModal_features_intro_user_id');
		var user = {};
		if (url.length) {
			...
			$.ajax({
				...
				success: function (responseText) {
					...
					$.each(htmlObj.find('th > a'), function (idx, el) {
						var el = $(el);
						var userName = el.attr('data-user-name');
						var userId = el.attr('data-user-value');
				...
				console.log(`code: ${res.status}: Can not get Response Text from ${url}`);
			})
		}

		$.each($('div#acm-sortable1').find('input.field-user-input'), function (idx, el) {
			var el = $(el);
			var userId = $.isEmptyObject(user) ? el.val() : user[el.val()];

			if (idx !== 0){
				$('input#features_intro_user_id_' + idx).attr('value', userId);
			}else{
				$('input#features_intro_user_id').attr('value', userId);
			}
		});
	})
...
jaToolsInit = function ($) {
    ...
    $allElems.each(function () {
        ...
        if ($this.hasClass('required')) {
            $this.attr('required', 'required').removeClass('required');
        }
